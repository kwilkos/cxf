...
#
# $HeadURL: http://svn.collab.net/repos/svn/branches/1.0.8/tools/hook-scripts/commit-email.pl.in $
# $LastChangedDate: 2004-02-12 05:07:35 -0500 (Thu, 12 Feb 2004) $
# $LastChangedBy: dlr $
...
# Branch actions. The %branch_actions hash can be filled in with specific
# branch names and associated actions to override the defaults for just
# those branches.
#
# example: my %branch_actions = ('foo_branch' => $branch_committer_email);
#
my ($branch_no_email, $branch_all_email, $branch_committer_email) = (1, 2, 3);
my $default_branch_action = $branch_all_email;
my %branch_actions = ('MAVEN2' => $branch_committer_email);
...
my ($no_email, $email_author_only);
my $firstlogline;
my @log;
while (@svnlooklines)
  {
    $_ = shift @svnlooklines;
    if (/^\s*$/)
      {
        push @log, "$_\n";
        next;
      }
    s/^(NOEMAIL|EMAILALL|EMAILME)\s+//;
    if ($1)
      {
        $no_email = ($1 eq 'NOEMAIL');
        $email_author_only = ($1 eq 'EMAILME');
      }
    $firstlogline = $_;
    push @log, "$_\n";
    last;
...
# See if the adds/dels/mods all involve a given tag or branch
my $tag_or_branch_prefix;
my $tag_or_branch_count = grep(m:^(tags|branches)/:, @adds, @dels, @mods);
if ($tag_or_branch_count && $total_adds_dels_mods == $tag_or_branch_count)
  {
    my $ln = $adds[0] || $dels[0] || $mods[0];
    my ($tag_or_branch, $tag_or_branch_name) =
        ($ln =~ m:^(tags|branches)/([^/]+):);
    $tag_or_branch =~ s/e?s//;
    $tag_or_branch_prefix = ucfirst($tag_or_branch) . " $tag_or_branch_name ";
    if ($tag_or_branch eq 'branch'
        && exists $branch_actions{$tag_or_branch_name})
      {
        my $action = $branch_actions{$tag_or_branch_name};
        $no_email = ($action == $branch_no_email);
        $email_author_only = ($action == $branch_committer_email);
      }
  }

# Get the diff from svnlook.
#my @no_diff_deleted = $no_diff_deleted ? ('--no-diff-deleted') : ();
#my @difflines = &read_from_process($svnlook, 'diff', $repos, '-r', $rev, @no_diff_deleted);
...
foreach my $project (@project_settings_list)
  {
    ...
    if ($commondir ne '')
      {
        $subject = "${tag_or_branch_prefix}Commit R$rev - $firstlogline";
      }
    else
      {
        $subject = "${tag_or_branch_prefix}Commit R$rev - $firstlogline";
    ...
    $userlist = $mail_from if $email_author_only;
