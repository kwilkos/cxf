...
public class WSDLToServiceProcessorTest extends ProcessorTestBase {
    ...
    public void testNewService() throws Exception {
        String[] args = new String[]{"-transport", "http", "-e", "serviceins",  
                                     "-p", "portins", "-n", "Greeter_SOAPBinding", 
                                     "-a", "http://localhost:9000/newservice/newport",
        ...
        assertTrue(serviceString.indexOf("<wsdl:port name=\"portins\" binding=\"tns:Greeter_SOAPBinding\">") 
                   >= 0);
        assertTrue(serviceString.indexOf(
                   "<soap:address location=\"http://localhost:9000/newservice/newport\"/>") >= 0);        
    }
    ...
    public void testDefaultLocation() throws Exception {        
        ...
        String serviceString = new String(sb);        
        assertTrue(serviceString.indexOf(
                    "<soap:address location=\"http://localhost:9000/serviceins/portins\"/>") >= 0);
    }
    ...
    public void testServiceExist() throws Exception {
        ...
        try {
            ...
            if (e instanceof ToolException 
                 && e.toString().indexOf("Input server and port already exist in imported contract.") >= 0) {
